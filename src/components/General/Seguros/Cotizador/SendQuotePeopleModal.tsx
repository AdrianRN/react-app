...
function DataGridContact(props: any) {
  ...
  const sendEmail = async () => {
    ...
    const QuoteData: QuoteDataBase = {
      ...
      totalSuretyAmount: props.GottenQuote.totalSuretyAmount,
      suretyAmount: 0, //Optional for no bonus
      bonus: props.GottenQuote.bonus,
      netBonus: 0, //Optional for no bonus
    ...
    await BondQuotationService.postBondQuotationSendEmail(QuoteData).then((response)=>{
      ...
      <Box sx={{ p: 5, mr: 10, ml: 10 }}>
        ...
        >
          ...
          <Button
            ...
            onClick={async () => sendQuotaHandle()}
            style={{ margin: "10px" }}
          >
            Enviar Cotización
          </Button>
        </Grid>
        {(waiting) ? <LoadingScreen message='Enviando email' /> : <></>}
      </Box>
    </>
  );
}
